  ...
  return (
    <div className="relative">
      <div className="glass-card rounded-3xl p-8 md:p-12 relative overflow-hidden transition-all duration-500">
        ...
        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-indigo-100 rounded-full opacity-50 blur-3xl"></div>
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 relative z-10">
          <div className="flex items-center justify-between w-full md:w-auto">
            <h1 className="text-3xl font-bold text-indigo-900 tracking-tight">
              Zen<span className="text-purple-600">Quotes</span>
            </h1>
            <button
              onClick={() => setShowBookmarks(true)}
              className="md:hidden flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-600 text-sm font-semibold hover:bg-indigo-100 transition-colors"
            >
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="flex items-center gap-2 w-full md:w-auto">
              <label htmlFor="category" className="text-sm font-medium text-indigo-700 whitespace-nowrap">Category:</label>
              <select
                id="category"
                value={category}
                onChange={handleCategoryChange}
                className="w-full md:w-auto bg-white border border-indigo-100 text-indigo-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block p-2.5 outline-none transition-shadow hover:shadow-sm"
              >
                {CATEGORIES.map(cat => (
            ...
            <button
              onClick={() => setShowBookmarks(true)}
              className="hidden md:flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-50 text-indigo-600 font-semibold hover:bg-indigo-100 transition-colors"
            >
        ...
        <main className="min-h-[220px] flex flex-col justify-center relative z-10">
          {loading ? (
            <div className="flex flex-col items-center justify-center space-y-4">
              <div className="w-12 h-12 border-4 border-indigo-200 loader rounded-full"></div>
              <p className="text-indigo-400 font-medium animate-pulse">Curating inspiration...</p>
            </div>
          ) : error ? (
            ...
            <div key={quote?.id} className="animate-quote-entrance text-center md:text-left">
              <div className="flex items-start justify-between">
                <svg className="w-10 h-10 text-indigo-200 mb-4 mx-auto md:mx-0" fill="currentColor" viewBox="0 0 32 32">
                ...
                <button
                  ...
                  className={`p-3 rounded-full transition-all duration-300 ${isCurrentBookmarked ? 'text-pink-500 bg-pink-50' : 'text-indigo-300 bg-indigo-50 hover:text-indigo-500'}`}
                >
              ...
              <blockquote className="text-2xl md:text-3xl font-serif font-bold text-indigo-950 leading-snug mb-6 italic">
                {quote?.text}
              </blockquote>
        ...
        <footer className="mt-12 flex flex-wrap items-center justify-center md:justify-between gap-4 relative z-10">
          ...
          <button
            onClick={handleNewQuote}
            disabled={loading}
            className={`px-8 py-3 rounded-full bg-indigo-600 text-white font-bold tracking-wide shadow-lg hover:bg-indigo-700 hover:shadow-indigo-200 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed`}
          >
      ...
      {showBookmarks && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-overlay animate-fade-in" onClick={() => setShowBookmarks(false)}>
          <div
            className="glass-card w-full max-w-xl max-h-[80vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl animate-modal-in"
            onClick={e => e.stopPropagation()}
          >
            <header className="p-6 border-b border-indigo-50 flex items-center justify-between">
              <h2 className="text-xl font-bold text-indigo-900 flex items-center gap-2">
                <svg className="w-6 h-6 text-indigo-600" fill="currentColor" viewBox="0 0 20 20"><path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-2.5L5 18V4z"></path></svg>
                Saved Quotes
              </h2>
              <button
                onClick={() => setShowBookmarks(false)}
                className="p-2 rounded-full hover:bg-indigo-50 text-indigo-400 transition-colors"
              >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {bookmarks.length === 0 ? (
                ...
                bookmarks.map(b => (
                  <div key={b.id} className="p-5 rounded-2xl bg-indigo-50/50 border border-indigo-50 group hover:border-indigo-200 transition-all">
                    <p className="text-indigo-950 font-serif leading-relaxed mb-3">"{b.text}"</p>
                    <div className="flex items-center justify-between">
                      <span className="text-indigo-600 text-sm font-semibold tracking-wide">— {b.author}</span>
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => copyToClipboard(b)}
                          className="p-1.5 rounded-lg text-indigo-400 hover:text-indigo-600 hover:bg-white transition-all"
                          title="Copy"
                        >
